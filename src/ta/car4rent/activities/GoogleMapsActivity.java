...
public class GoogleMapsActivity extends ActionBarActivity implements
	...
	boolean stopRefreshComent = false;
	// ==============================================
	// COMENT
	// ==============================================
	...
	public void getWarningFromServer(PostWarningFragment PostFragment) {
		...
		serviceWarningGet.getAllWarningNews(
				GoogleMapsActivity.mCurrentLocation.longitude,
				GoogleMapsActivity.mCurrentLocation.latitude, ConfigureData.radiusWarning);
	...
	private void fillWarningIntoSlidingUp(JSONObject objectClicked) {
		...
		try {
			...
			String imageUrl = objectClicked.getString("imageUrl");
			if (imageUrl != null) {
				ConfigureData.imageLoader.displayImage(imageUrl, imageWarning,
						250);
			} else {
		...
		serviceGetComent.addOnGetJsonListener(new OnGetJsonListener() {
			...
			public void onGetJsonCompleted(String response) {
				...
				try {
					JSONObject responseJson = new JSONObject(response);
					if (responseJson.getBoolean("status")) {
						comentArray = responseJson.getJSONArray("data");
						if (commentAdapter != null
								&& comentArray.length() == commentAdapter
										.getCount()) {

						} else {
							commentAdapter = new ComentArrayAdapter(
									GoogleMapsActivity.this, comentArray);
	...
	private OnClickListener loadDataWarning_clickListener = new OnClickListener() {
		public void onClick(View v) {
			...
			warningsManager.drawWarningsIntoMap(warningsManager.onlyShowWarningsInMaps, googleMap);
		}
	...
	private ListPopupWindow showPopupListViewWarning(Context mcon,
		...
		int height = size.y;
		/*
		 * pwindo.setWidth(width - 50); pwindo.setHeight(height - 200);
		 */
